...
sink("S:/etude25/2679_Danieli_These/Review/Test_PH_nsim1000/lin_ph_lambda0.colon_0.02_2000/100000_patients/temps_exe_new3.lis")

chemin     <- ("S:/etude25/2679_Danieli_These/Review/big_sample_size/")
chemin.bis <- ("S:/etude25/2679_Danieli_These/Review/big_sample_size/Test_PH_nsim1000/")
chemin2    <- ("Simulations/lin_ph_lambda0.colon_0.02_2000/100000_patients/")
...
load( paste( chemin , chemin2 , "ListDataSimulation.lin.ph.RData" , sep="" ) )
...
for ( Fich in 1:NbFichier )
{
  ...
  tempdonnees = ListDataSimulation.lin.ph[[Fich]]
  ...
  tempdonnees$CAUSE           <- tempdonnees$cause
  
  # ----- > 10 years of follow-up
  
  tempdonnees$ETAT            <- ifelse( tempdonnees$SUIVI.A > 10         , 0        , tempdonnees$ETAT)
  tempdonnees$CAUSE           <- ifelse( tempdonnees$SUIVI.A > 10         , 0        , tempdonnees$CAUSE)
  ...
  print(dim(tempdonnees))
  print(head(tempdonnees))
  ...
  Tps.Unique.J <- Tps.Unique * 365.24
...
cat("\nPreparation (2)\n")
  ...
  My.Mat.pDX.t <- matrix(0,n.pat,n.cov)
  for (j in 1:n.cov)
  {
      My.List.Hess.X.t[[j]] <- matrix(0,n.tps,leCoef)
  }
  
  Temp.IMT      <- sapply(1:n.tps,function(x){max(which(Idx.MyTime<=x))})
  Temp.IMT.Rev <- sapply(1:n.tps,function(x){min(which(Idx.MyTime>=x))})

  MyT1 <- matrix(0,n.tps,5)

for (j in 1:n.cov)
{
    Temp1 <- Covariables[,j]*exp.beta.z

    TempPart1 <- cumsum(Temp1*Frac.lambda.d1*d1.Part2[,1])[Temp.IMT]
    TempPart2 <- apply(apply(d1.Part2,2,function(x){(Temp1*Frac.lambda.d2)*x}),2,cumsum)[Temp.IMT,]

    RT1 <- rev(Temp1)
    for (l in 1:5)
    {
        MyT1[,l] <- (c(0,cumsum(Temp1*d1.Part1[Idx.MyTime,l])[Temp.IMT])+c(sapply(1:n.tps,function(x){cumsum(RT1*d1.Part1[x,l])[n.pat+1-Temp.IMT.Rev[x]]}),0))[-(n.tps+1)]
    }
    My.Score.X.t[,j] <- TempPart1 - MyT1[,1]
    My.List.Hess.X.t[[j]][,1:5] <- MyT1 - TempPart2
    for (k in 1:n.cov)
    {
        Temp2 <- Covariables[,k]*Temp1
        RT2 <- rev(Temp2)
        Temp3 <- cumsum(Temp2[,n.cov*(j-1)+k]*Frac.lambda.d2)[Temp.IMT]
        Temp4 <- (c(0,cumsum(Temp2*d1.Part1[Idx.MyTime,1])[Temp.IMT])+c(sapply(1:n.tps,function(x){cumsum(RT2*d1.Part1[x,1])[n.pat+1-Temp.IMT.Rev[x]]}),0))[-(n.tps+1)]
        My.List.Hess.X.t[[j]][,5+k] <- Temp4 - Temp3
    }
    My.Mat.pDX.t[,j] <- Temp1*Frac.lambda.d1
}
  ...
  My.Gaussian.Process.PH <- function(n.obs,pD.tau,pDX.t,RowHess.Xt.Sigma,Idx.pDx)
  {
      G  <- rnorm(n.obs,0,1)
      D.tau <- t(G%*%pD.tau)
      DX.t <- cumsum(G*pDX.t)[Idx.pDx]
  ...
  for (j in 1:n.cov)
  {
      ...
      My.pDX.t <- My.Mat.pDX.t[,j]
      My.RowHess.Xt.Sigma <- My.List.Hess.X.t[[j]]%*%My.Sigma
 
     for (k in 1:n.sim)
     {
          Temp.Gauss.PH[,k] <- My.Gaussian.Process.PH(n.obs=n.pat,pD.tau=My.pD.tau,pDX.t=My.pDX.t,
                                                RowHess.Xt.Sigma=My.RowHess.Xt.Sigma,Idx.pDx=Temp.IMT)
      }
