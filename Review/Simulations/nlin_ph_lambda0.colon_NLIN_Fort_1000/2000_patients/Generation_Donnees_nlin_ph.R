...
chemin2 <- c("Simulations/nlin_ph_lambda0.colon_NLIN_Fort_1000/2000_patients/")
...
load( file = paste( chemin , "Simulations/Parametres_Theoriques/nlin_ph_prostate/parametre_theorique/coef.prostate.nlin.ph.Rdata" , sep = "" ) )

coef.prostate.nlin.ph[1:5] <- coef.colon.lin.nph[1:5]
...
for (i in 1:NbFichier)
{
  ...
  print(Sys.time())
  ListDataSimulation.nlin.ph[[i]] <- cdatasimulation.PourDiffusion( donnees = DataDesign.nlin.ph , beta = coef.prostate.nlin.ph , modele.simule = DataDesign.nlin.ph$mod.nlin.ph , cens.admin = 2013 , min.adiag = 1990 )
}
