...
chemin  <- c("C:/Users/Manu/Desktop/HCL/Github_LRe_3/Review/")
chemin2 <- c("Simulations/lin_nph_lambda0.colon_NPH_Moyen_1000/2000_patients/")
...
coef.colon.lin.nph.1 <- coef.colon.lin.nph
load( file = paste( chemin , "Simulations/Parametres_Theoriques/lin_nph_colon/parametre_theorique/coef.colon.lin.nph.Rdata" , sep = "" ) )

coef.colon.lin.nph[1:5] <- coef.colon.lin.nph.1[1:5]
...
for (i in 1:NbFichier)
{
  ...
  print(Sys.time())
  ListDataSimulation.lin.nph[[i]] <- cdatasimulation.PourDiffusion( donnees = DataDesign.lin.nph , beta = coef.colon.lin.nph , modele.simule = mod.lin.nph , cens.admin = 2013 , min.adiag = 1990 )
}
