...
Modele.v2 <- function( donnees = tempdonnees )
{
  ...
  my.model = "~ intnum + I(intnum^2) + I( (intnum-1)^2 * (intnum > 1) ) + I( (intnum-5)^2 * (intnum > 5) ) + ( intnum + I(intnum^2) + I( (intnum-1)^2 * (intnum > 1) ) + I( (intnum-5)^2 * (intnum > 5) ) ) * agec"
  ...
  splitdon        = splitdon[,c("AGE.DIAG.A","AGEC.DIAG.A","ANNEE.DIAG.A","ANNEEC.DIAG.A","Fail","tik","intnum","dcatt","MUA","Entry","Exit")]
  splitdon$unsurt = 1/(splitdon$intnum + 1 )
  splitdon$logt   = log( splitdon$intnum + 1 )
  splitdon$agec   = trunc(splitdon$AGEC.DIAG.A)
  splitdon$adiagc = trunc(splitdon$ANNEEC.DIAG.A)
