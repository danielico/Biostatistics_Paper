...
excess.rate.fp.ml.v4.vagec = function( splitdonnees , modele , theta.init )
{
  ...
  p = ncol( model.matrix( as.formula(modele) , data.frame( intnum = 1 , agec = 1 , agec2 = 1 , agec3 = 1 , agec3plus = 1 , adiagc = 1 , unsurt = 1 , logt = 1) ) ) #nb de cov ds le modele
  ...
  agec2     <- splitdonnees$agec2
  agec3     <- splitdonnees$agec3
  agec3plus <- splitdonnees$agec3plus
	adiagc <- splitdonnees$adiagc
	unsurt <- splitdonnees$unsurt
	logt   <- splitdonnees$logt

	X.tm = model.matrix( as.formula(modele) , data.frame( intnum = tm , agec = agec , agec2 = agec2 , agec3 = agec3 , agec3plus = agec3plus , adiagc = adiagc , unsurt = unsurt , logt = logt ) )
	X.t0 = model.matrix( as.formula(modele) , data.frame( intnum = t0 , agec = agec , agec2 = agec2 , agec3 = agec3 , agec3plus = agec3plus , adiagc = adiagc , unsurt = unsurt , logt = logt ) )
	X.t1 = model.matrix( as.formula(modele) , data.frame( intnum = t1 , agec = agec , agec2 = agec2 , agec3 = agec3 , agec3plus = agec3plus , adiagc = adiagc , unsurt = unsurt , logt = logt ) )
...
if ( fit == T )
{
  ...
  names(theta) = dimnames( model.matrix( as.formula(modele) , data.frame( intnum = 1 , agec = 1 , agec2 = 1 , agec3 = 1 , agec3plus = 1 , adiagc = 1 , unsurt = 1 , logt = 1 ) ) )[[2]]
