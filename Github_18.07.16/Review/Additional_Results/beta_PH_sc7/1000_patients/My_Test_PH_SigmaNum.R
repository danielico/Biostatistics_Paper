# ------------------------------------------------------------------ #
# -- PH test : Fit NLIN-PH model on LIN-NPH simulated data-- #
# ------------------------------------------------------------------ #
...
sink("S:/etude25/2679_Danieli_These/Review/Additional_Results/beta_PH_sc7/1000_patients/My_Test_PH_SigmaNum.lis")
...
chemin.bis <- ("S:/etude25/2679_Danieli_These/Review/Additional_Results/beta_PH_sc7/")
chemin2    <- ("Simulations/lin_nph_lambda0.colon_NPH_Faible_1000/1000_patients/")
chemin3    <- ("Additional_Results/beta_PH_sc7/1000_patients/")
...
for ( Fich in 1:NbFichier )
{
  ...
  tempdonnees$AGEC.DIAG.A2    <- tempdonnees$agec^2
  tempdonnees$AGEC.DIAG.A3    <- tempdonnees$agec^3
  tempdonnees$AGEC.DIAG.A3plus<- ((tempdonnees$agec - mean(tempdonnees$agec))^3) * (tempdonnees$agec > mean(tempdonnees$agec))
  ...
  Covariables <- model.matrix( ~ -1 + AGEC.DIAG.A + AGEC.DIAG.A2 + AGEC.DIAG.A3 + AGEC.DIAG.A3plus , data = MyData ) 
...
for (j in 1:n.cov)
{
  ...
  Temp.Gauss.PH.list[[j]] <- Cst.Mult * Temp.Gauss.PH
}

Temp.Gauss.PH    <- Temp.Gauss.PH.list[[1]] + Temp.Gauss.PH.list[[2]] + Temp.Gauss.PH.list[[3]] + Temp.Gauss.PH.list[[4]]

Mat.max.PH       <- apply(abs(Temp.Gauss.PH),2,max)

statistic.test   <- max( apply( abs( My.Score.X.t * Cst.Mult ) , 1 , sum ) )

p.value.PH       <- sum( Mat.max.PH >= statistic.test )/n.sim
