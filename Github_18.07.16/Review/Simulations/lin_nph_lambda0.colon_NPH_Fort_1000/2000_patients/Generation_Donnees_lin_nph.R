...
chemin2 <- c("Simulations/lin_nph_lambda0.colon_NPH_Fort_1000/2000_patients/")
...
load( file = paste( chemin , "Simulations/Parametres_Theoriques/lin_nph_thyroide/parametre_theorique/coef.thyroide.lin.nph.Rdata" , sep = "" ) )

coef.thyroide.lin.nph[1:5] <- coef.colon.lin.nph[1:5]
...
for (i in 1:NbFichier)
{
  ...
  print(Sys.time())
  ListDataSimulation.lin.nph[[i]] <- cdatasimulation.PourDiffusion( donnees = DataDesign.lin.nph , beta = coef.thyroide.lin.nph , modele.simule = mod.lin.nph , cens.admin = 2013 , min.adiag = 1990 )
}
