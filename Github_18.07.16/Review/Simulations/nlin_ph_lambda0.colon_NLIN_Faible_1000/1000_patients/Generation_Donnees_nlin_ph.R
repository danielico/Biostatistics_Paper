...
chemin  <- c("C:/Users/Manu/Desktop/HCL/30.06.16/Review/")
chemin2 <- c("Simulations/nlin_ph_lambda0.colon_NLIN_Faible_1000/1000_patients/")
...
load( file = paste( chemin , "Simulations/Parametres_Theoriques/nlin_ph_colon/parametre_theorique/coef.colon.nlin.ph.Rdata" , sep = "" ) )

coef.colon.nlin.ph[1:5] <- coef.colon.lin.nph[1:5]
...
for (i in 1:NbFichier)
{
  ...
  print(Sys.time())
  ListDataSimulation.nlin.ph[[i]] <- cdatasimulation.PourDiffusion( donnees = DataDesign.nlin.ph , beta = coef.colon.nlin.ph , modele.simule = DataDesign.nlin.ph$mod.nlin.ph , cens.admin = 2013 , min.adiag = 1990 )
}
